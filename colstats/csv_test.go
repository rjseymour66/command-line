...
func TestOperations(t *testing.T) {
	...
	/*
		Loop through each test case. In each each test case, run a loop where the result
		is the statsFunc on the input data. Compare result to the exp in the test case
	*/
...
`
	...
	testCases := []struct {
		...
		// iotest.TimeoutReader simulates a reading failure and returns a timeout error
		{name: "FailRead", col: 1,
			exp:    nil,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr != nil {
				...
				// prevent the execution of remaining checks
				return
			}
			...
			for i, exp := range tc.exp {
				if res[i] != exp {
					t.Errorf("Expect %g, got %g instead", exp, res[i])
				}
			}
		})
	}
